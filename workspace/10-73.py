...
def build_mboum_price_url(symbol: str) -> str:
 ...
 if custom:
 return custom.replace(\"{{symbol}}\", symbol).replace(\"{{SYMBOL}}\", symbol)
...
def fetch_mboum_price(symbol: str) -> tuple[float | None, str | None]:
 \"\"\"Call MBOUM API directly and return (price, error_or_None).\"\"\"
 try:
 ...
 url = build_mboum_price_url(symbol)
 ...
 # meta.regularMarketPrice is the real-time field
 if isinstance(payload, dict):
...
def run_single(symbol: str | None, context: dict[str, Any] | None, args: argparse.Namespace) -> dict[str, Any]:
 ...
 # Always attempt a live MBOUM API call — context may be stale or missing.
 live_mboum_price, live_mboum_error = fetch_mboum_price(symbol)
 ...
 volume = yfinance_snapshot.get(\"volume\")
 avg_volume = yfinance_snapshot.get(\"avg_volume\")
 vol_change_pct = yfinance_snapshot.get(\"vol_change_pct\")
 yfinance_price = yfinance_snapshot.get(\"price\")
 price = (
 mboum_price if mboum_price is not None
 else yfinance_price if yfinance_price is not None
 else fintel_price if fintel_price is not None
 else massive_price
 )
 price_source = (
 \"mboum\" if mboum_price is not None
 else \"yfinance\" if yfinance_price is not None
 else (fintel_price_source if fintel_price is not None else massive_price_source)
 )
 price_change_pct = yfinance_snapshot.get(\"price_change_pct\")
 beta = yfinance_snapshot.get(\"beta\")
 ...
 short_float_display_pct = yfinance_short_pct
 ...
 return {
 \"code\": \"10-73\",
 \"data_sources\": [\"mboum_realtime\", \"mboum_v2_short_interest\", \"yfinance\", \"fintel_proxy\"],
 \"NOT_FROM\": \"finviz\",
...
def print_single(payload: dict[str, Any]) -> None:
 ...
 print(f\"10-73 {symbol} {now_local} | DATA: mboum+yfinance+fintel | NOT finviz\")
 ...
 print(f\" SI [yfinance]: {format_pct(payload.get('yfinance_short_percent_of_float_pct'))}\")
 print(f\" DTC [mboum_v2]: {format_plain(payload.get('mboum_si_days_to_cover'))}\")
 print(f\" DTC [yfinance]: {format_plain(payload.get('yfinance_days_to_cover'))}\")
 print(f\" Vol [yfinance]: {format_number_compact(payload.get('yfinance_volume'))} ({format_signed_pct(payload.get('yfinance_vol_change_pct'))})\")
 ...
 print(f\" Beta [yfinance]: {format_plain(payload.get('yfinance_beta'))}\")
 print(f\" SI delta [yfinance]: {format_signed_pct(payload.get('yfinance_si_change_pct'))}\")
 print(f\" Signal: {payload.get('cmo_signal','?')}\")
 print(f\" Verdict [{payload.get('short_float_source','?')} vs yfinance]: {payload.get('verdict','?')} | diff={format_pp(payload.get('abs_diff_pp'))}\")
 if payload.get('yfinance_error'):
 print(f\" WARN yfinance: {payload['yfinance_error']}\")
 if payload.get('live_mboum_error'):
 print(f\" WARN mboum_live: {payload['live_mboum_error']}\")
...
def main() -> int:
 ...
 # Keep backwards compat: single-symbol path uses existing context (already loaded above).
